"""
技术指标计算模块
提供基金/股票常用技术指标的计算函数，基于 numpy 实现。
"""
...
def std_dev(values: np.ndarray, period: int = 20) -> np.ndarray:
    """滚动标准差。"""
    result = np.full_like(values, np.nan, dtype=float)
    if len(values) < period:
        return result
