...
class FundViewerApp:
    ...
    def _build_ui(self):
        ...
        for text, val in [('单位净值', 'nav'), ('累计净值', 'acc_nav'), ('日收益率', 'daily_chg'),
                          ('近3天收益', 'ret_3d'), ('近1周收益', 'ret_1w'), ('近1月收益', 'ret_1m')]:
            ttk.Radiobutton(ctrl, text=text, variable=self.nav_mode_var,
                             value=val,
                             command=lambda: self._switch_group('nav')).pack(side=tk.LEFT, padx=2)
        ...
        for text, val in [('K线+均线', 'kline'), ('布林带', 'boll'),
            ...
            ttk.Radiobutton(ctrl, text=text, variable=self.ohlc_mode_var,
                             value=val,
                             command=lambda: self._switch_group('ohlc')).pack(side=tk.LEFT, padx=2)
    ...
    def _draw_nav_chart(self, code, mode):
        ...
        if mode == 'nav':
        ...
        elif mode in ('ret_3d', 'ret_1w', 'ret_1m'):
            window_map = {'ret_3d': 3, 'ret_1w': 5, 'ret_1m': 22}
            title_map = {'ret_3d': '近3天滚动收益率', 'ret_1w': '近1周滚动收益率', 'ret_1m': '近1月滚动收益率'}
            window = window_map[mode]
            title = title_map[mode]

            unit_navs = [d['unit_nav'] for d in nav_data]
            # 计算滚动收益率: (当前NAV / N天前NAV - 1) * 100%
            rolling_rets = []
            for i in range(len(unit_navs)):
                if i < window or unit_navs[i - window] is None or unit_navs[i - window] == 0:
                    rolling_rets.append(None)
                else:
                    ret = (unit_navs[i] / unit_navs[i - window] - 1) * 100
                    rolling_rets.append(ret)

            vals = [r for r in rolling_rets if r is not None]
            if vals:
                plot_dates = dates[len(dates) - len(vals):]
            else:
                plot_dates = []

            colors = [C['green'] if v >= 0 else C['red'] for v in vals]
            ax.bar(plot_dates, vals, width=2, color=colors, alpha=0.7)
            ax.axhline(y=0, color=C['txt2'], linewidth=0.5, alpha=0.5)
            ax.set_ylabel(f'{window}日滚动收益率 (%)', color=C['txt2'], fontsize=9)
            ...
            if vals:
                ...
                max_val = _np.max(arr)
                min_val = _np.min(arr)
                pos_ratio = _np.sum(arr > 0) / len(arr) * 100
                stats_text = (f'平均: {avg:.2f}%  |  最大: {max_val:.2f}%  |  '
                               f'最小: {min_val:.2f}%  |  正收益占比: {pos_ratio:.1f}%')
                ax.text(0.01, 0.97, stats_text, transform=ax.transAxes,
                        fontsize=8, color=C['txt2'], va='top',
